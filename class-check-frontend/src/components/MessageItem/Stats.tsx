...
export function MessageStats({ messageId }: MessageStatsProps) {
  ...
  return (
    ...
    >
      {data?.stats.recipientsCount && (
        <Flex alignItems="center">
          <Progress
            ...
            colorScheme="pink"
            value={
              ((data?.stats.deliverCount || 0) / data?.stats.recipientsCount) *
              100
            }
          />
          <Text color="gray.500" ml="2">
            {`${data?.stats.deliverCount || 0}/${
              data?.stats.recipientsCount
            } destinatários`}
          </Text>
        </Flex>
      ...
      {data?.stats.openRate && (
        <Text color="gray.500">{`${data?.stats.openRate}% de abertura`}</Text>
      )}
      {data?.stats.clickRate && (
        <Text color="gray.500">{`${data?.stats.clickRate}% de clicks`}</Text>
      )}
      {data?.stats.clickCount && (
        <Text color="gray.500">{`${data?.stats.clickCount} clicks`}</Text>
      )}
    </HStack>
  );
}
